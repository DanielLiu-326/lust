...
pub fn generate_fn<'gc, A:Allocator>(root:FnDescriptor, consts:&mut Vec<Constant<'gc>>, codes: &mut Vec<OpCode>, hdl:MutateHandle<'gc, '_, A>)
    ->FnProto<'gc>
{
    let pc = codes.len();
    ...
    FnProto::alloc(meta,SliceIterInitializer::new(root.up_values.len(),root.up_values.iter().map(|up|{
        match up{
