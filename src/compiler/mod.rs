...
use crate::compiler::descriptors::{
    ConstantDescriptor, ExprDescriptor, ExprResPos, FnDescriptor, IdentPos, LeftExprDescriptor,
...
pub fn compile_vector_literal(
    ...
    for mem in members {
        ...
        let ExprDescriptor {
            res: expr_res,
            codes: mut expr_codes,
        } = compile_expr(scope, mem, None)?;
        codes.append(&mut expr_codes);
        codes.push(OpCodeExt::OpCode(OpCode::Add(reg, reg, expr_res.into())));
    }
    return Ok(VectorDescriptor { res: reg, codes });
}
...
pub fn compile_expr_fn_call(
    ...
    //TODO： temporary print implementation
    {
        if let Expr::Ident("print") = callee {
            ...
            return Ok(ExprDescriptor { res, codes });
        }
    }
    ...
    codes.push(OpCodeExt::OpCode(OpCode::Call(
        callee_reg,
        arg_num as Register,
        1,
    )));
...
pub fn compile_expr_index_visit(
    parent_scope: Rc<RefCell<dyn Scope>>,
    left: Expr,
    index: Expr,
) -> Result<ExprDescriptor> {
    ...
    let res = if let ExprResPos::New(reg) = obj {
        ...
        reg
    } else if let ExprResPos::New(reg) = idx {
        reg
    } else {
        parent_scope.borrow_mut().reg_alloc().allocate().unwrap() // TODO unwrap
    };

    codes.push(OpCodeExt::OpCode(OpCode::GetMember(
        res,
        obj.into(),
        idx.into(),
    )));
    Ok(ExprDescriptor {
        res: ExprResPos::New(res),
        codes,
    })
}
...
pub fn compile_left_expr_ident(
    parent_scope: Rc<RefCell<dyn Scope>>,
    id: Ident,
...
pub fn compile_left_expr_member(
    parent_scope: Rc<RefCell<dyn Scope>>,
    prefix: Expr,
    index: Expr,
) -> Result<LeftExprDescriptor> {
...
pub fn compile_left_expr(
    parent_scope: Rc<RefCell<dyn Scope>>,
    expr: LeftExpr,
) -> Result<LeftExprDescriptor> {
    match expr {
        LeftExpr::Ident(id) => compile_left_expr_ident(parent_scope, id),
        LeftExpr::Member { prefix, index } => compile_left_expr_member(parent_scope, prefix, index),
    }
}
...
pub fn compile_var_define_stmt(
    ...
    let ExprDescriptor { res, mut codes } = compile_expr(parent_scope.clone(), right, Some(reg))?;
...
pub fn compile_if_stmt(
    ...
    let success_start = parent_scope
        ...
        .borrow_mut()
        .def_anonymous();
    let success_end = parent_scope
        .borrow_mut()
        .label_mgr()
        .borrow_mut()
        .def_anonymous();
    fail_codes.push(OpCodeExt::Goto(success_end));
    codes.push(OpCodeExt::OpCode(OpCode::TestFalse(
        cond_res.clone().into(),
    )));
...
pub fn compile_ret_stmt(
    ...
    let ExprDescriptor {
        res: expr_reg,
        codes: mut expr_codes,
    } = compile_expr(parent_scope.clone(), expr, None)?;
    codes.append(&mut expr_codes);
...
pub fn compile_stmt(parent_scope: Rc<RefCell<dyn Scope>>, stmt: Stmt) -> Result<StmtDescriptor> {
    match stmt {
        ...
        Stmt::VarDefineStmt { ident, right } => compile_var_define_stmt(parent_scope, ident, right),
        Stmt::ConstDefineStmt { .. } => {
            todo!()
        }
        Stmt::NormalStmt(expr) => compile_normal_stmt(parent_scope, expr),
        Stmt::WhileStmt { cond, body } => compile_while_stmt(parent_scope, cond, *body),
        Stmt::LoopStmt(_) => {
            todo!()
        }
        Stmt::BlockStmt(block) => compile_block_stmt(parent_scope, block),
        Stmt::ReturnStmt(expr) => compile_ret_stmt(parent_scope, expr),
        Stmt::Label(_, _) => {
            todo!()
        }
    }
...
pub mod test_compiler {
    ...
    pub(crate) fn test() {
        ...
        let func: crate::compiler::descriptors::FnDescriptor =
            compile_fn_literal(file_scope, Vec::new(), ast.stmts).unwrap();
        println!("compile result");
        println!("constant table:{:?}", func.consts);
        println!("extended opcode:{:?}", func.codes);

        println!("===========OpCode==========");
        let mut gc: GarbageCollector<Global, RootableTy![VM<'gc>]> =
