...
use syn::{GenericArgument, GenericParam, ItemStruct};
...
pub fn derive_collectable_fn(input: TokenStream) -> TokenStream {
    ...
    let item_struct: ItemStruct =
        syn::parse(input.clone()).expect("gc_derive: only support struct type for now");
    ...
    let struct_generic_arguments =
        struct_generic_params
            .iter()
            .fold(Punctuated::new(), |mut pun, param| {
                let arg = match param {
                    GenericParam::Type(a) => {
